...
router.get('/:id', async (req, res) => {
    try {
        ...
        return res.send({
            data: await req.app.locals.fests.find({
...
router.post('/', async (req, res) => {
    try {
        ...
        if (!Array.isArray(req.body.socialNetworks)) {
            validationErrorArray.push(`prop socialNetworks has incorrect type ${typeof req.body.socialNetworks}, but it should be Array`);
            return res.status(400).send({
                error: validationErrorArray
            });
        }
        ...
        req.body.socialNetworks.forEach(socialNetworkItem => {
            socialNetWorksErrorArray.push(validation(socialNetworksRequiredProps, socialNetworkItem));
        });
        ...
        if (passedArtistIds.filter(item => Boolean(item)).length) {
            artists = await req.app.locals.artists.find({
                _id: {
        ...
        const socialNetworks = req.body.socialNetworks.map(
            item => ({
                _id: new ObjectID(),
                name: item.name,
                link: item.link,
